...
describe('fetchWithTimeout', () => {
  ...
  it('retries on 502, 503, 504 status codes', async () => {
    ...
    const promise = fetchWithTimeout('http://example.com/api/test', { retries: 2 });
    await jest.runAllTimersAsync();
    const result = await promise;
  ...
  it('passes Next.js config when provided', async () => {
    ...
    const promise = fetchWithTimeout('http://example.com/api/test', {}, nextConfig);
    await jest.runAllTimersAsync();
    await promise;
  ...
  it('passes custom fetch config', async () => {
    ...
    const promise = fetchWithTimeout('http://example.com/api/test', customConfig);
    await jest.runAllTimersAsync();
    await promise;
...
describe('fetchWithTimeout - retry logic', () => {
  ...
  it('caps backoff at MAX_RETRY_DELAY', async () => {
    ...
    const promise = fetchWithTimeout('http://example.com/api/test', { retries: 3 });
  ...
  it('parses Retry-After header correctly', async () => {
    ...
    const promise = fetchWithTimeout('http://example.com/api/test', { retries: 1 });
    await jest.runAllTimersAsync();
    const result = await promise;
